...
import { AI_ENABLED, generateAdvisorPicks, type CandidateInput } from "./aiService";
...
export async function registerRoutes(
  ...
  // One-click, plain-English shortlist across all strategies' latest signals,
  // enriched with live indicators, explained by an AI model. Runs as a
  // background job (like the heavy scans) so the request never blocks long
  // enough to hit Render's gateway timeout.
  app.get("/api/status/ai-analyst", (_req, res) => {
    res.json(scanJobStatus("ai-analyst"));
  });

  app.post("/api/advisor/analyze", requireAuth, (_req, res) => {
    if (!AI_ENABLED) {
      return res.status(503).json({ message: "AI Analyst is not configured on this server." });
    }

    const { started, job } = startScanJob("ai-analyst", async (report) => {
      report(0, 4);

      // 1. Aggregate the latest signals across all strategies.
      const allSignals = await storage.getSignals(500);
      report(1, 4);
      if (allSignals.length === 0) {
        return {
          ...
          picks: [],
          note: "No recent scan signals found. Run a strategy scan first (e.g. the Live Scanner), then try the AI Analyst again.",
        };
      }

      // Group by symbol: collect which strategies flagged it and the latest
      // BUY-leaning price, so the same stock isn't sent to the AI 5 times.
      const bySymbol = new Map<string, { companyName: string; price: number; strategies: Set<string>; buyCount: number; sellCount: number }>();
      for (const s of allSignals) {
        ...
        if (s.signal === "BUY") entry.buyCount++;
        if (s.signal === "SELL") entry.sellCount++;
        entry.price = s.price || entry.price;
        bySymbol.set(s.symbol, entry);
      }
      report(2, 4);

      // Rank by number of distinct strategies agreeing, take the top 10 to
      // enrich with live data (keeps the AI payload compact and fast).
      ...
      for (const r of ranked) {
        try {
          const [quote, historicalData] = await Promise.all([
            getStockQuote(r.symbol),
            getHistoricalData(r.symbol),
          ]);
          if (!quote) continue;
          const rsi = historicalData.length ? calculateRSI(historicalData) : undefined;
          const dma124 = historicalData.length ? calculateDMA(historicalData, 124) : undefined;
          const srtv = dma124 ? calculateSRTV(quote.price, dma124) : undefined;
          candidates.push({
            symbol: r.symbol,
            companyName: quote.name || r.companyName,
            price: quote.price,
            strategiesFlagging: Array.from(r.strategies),
            signal: r.buyCount >= r.sellCount ? "BUY" : "SELL",
            rsi,
            dma124,
            srtv,
          ...
          // Skip symbols whose live data can't be fetched right now.
        }
      }
      report(3, 4);
      ...
      if (candidates.length === 0) {
        return {
          ...
          disclaimer:
            "Educational analysis generated by AI from historical/technical signals. Not investment advice. Markets carry risk. Consult a SEBI-registered advisor before investing.",
          picks: [],
          note: "Live market data could not be fetched right now. Please try again shortly.",
        };
      }

      const picks = await generateAdvisorPicks(candidates);
      report(4, 4);
  ...
  // Saved AI analyses (per signed-in user)
  app.get("/api/advisor/saved", requireAuth, async (req, res) => {
    try {
      const items = await storage.getSavedAnalyses(getUserId(req));
      res.json(items.map((i) => ({ ...i, picks: JSON.parse(i.picks) })));
    } catch (error: any) {
      log(`Error getting saved analyses: ${error.message}`);
      res.status(500).json({ message: "Failed to get saved analyses" });
    }
  });

  app.post("/api/advisor/saved", requireAuth, async (req, res) => {
    try {
      const userId = getUserId(req);
      const parsed = insertSavedAnalysisSchema.parse({
        generatedAt: new Date(req.body.generatedAt || Date.now()),
        picks: JSON.stringify(req.body.picks || []),
      });
      const saved = await storage.saveAnalysis(userId, parsed);
      res.json({ ...saved, picks: JSON.parse(saved.picks) });
    } catch (error: any) {
      log(`Error saving analysis: ${error.message}`);
      res.status(400).json({ message: error.message || "Failed to save analysis" });
    }
  });

  app.delete("/api/advisor/saved/:id", requireAuth, async (req, res) => {
    try {
      await storage.deleteSavedAnalysis(getUserId(req), req.params.id as string);
      res.json({ success: true });
    } catch (error: any) {
      log(`Error deleting saved analysis: ${error.message}`);
      res.status(500).json({ message: "Failed to delete saved analysis" });
    }
  });
